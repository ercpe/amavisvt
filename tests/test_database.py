...
class DummyVTResult(VTResponse):
    def __init__(self, infected):
        d = {
            ...
            "total": 40,
            "scans": {
                "Symantec": {"detected": True, "version": "20101.1.0.89", "result": "Trojan.KillAV",
                             "update": "20100515"},
            },
...
class TestAmavisVTDatabase(object):
    ...
    def test_get_filenames(self, testdb):
        ...
        with DBConnAndCursor() as (conn, cursor):
            ...
            cursor.execute(
                "INSERT INTO filenames (filename, pattern, infected, timestamp, sha256) VALUES ('bar', 'bar', 0, 0, 'bar')")
            cursor.execute(
    ...
    def test_clean(self, testdb):
        ...
        with DBConnAndCursor() as (conn, cursor):
            cursor.execute(
                "INSERT INTO filenames (filename, pattern, infected, timestamp, sha256) VALUES ('foo', 'bar', 0, 0, 'baz')")
    ...
    def test_filename_pattern_match_no_infected_pattern(self, testdb):
        with DBConnAndCursor() as (conn, cursor):
            for filename in ['foo-bar-1.zip', 'foo-bar-2.zip', 'foo-bar-3.zip']:
                cursor.execute(
                    "INSERT INTO filenames (filename, pattern, infected, timestamp, sha256) VALUES (?, 'foo-bar-[RANDOM]-zip', 0, 0, ?)",
                    (filename, filename))
            conn.commit()
